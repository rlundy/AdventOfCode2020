    ...
    module Day1 =
        ...
        let part1 (input : string) =
            let inputNumbers = input.Split ','
                               |> Seq.map (fun x -> Int64.Parse x)
            seq { for x in inputNumbers do
                  for y in inputNumbers do
                  yield x, y }
            |> Seq.filter (fun (x, y) -> x < y)
            |> Seq.filter (fun (x, y) -> x + y = 2020L)
            |> Seq.exactlyOne
            |> fun x -> fst x * snd x
